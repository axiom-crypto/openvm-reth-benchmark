            ...
            match account {
                Some(account) => {
                    ...
                    let storage_root = {
                        ...
                        let storage_trie = storage_tries.get_mut(hashed_address).unwrap();
...
impl StatelessTrie for EthereumState {
                ...
                if let Some(account) = &account_opt {
                    ...
                    if let Some(ref rlp_by_digest) = *self.rlp_by_digest.borrow() {
                        ...
                        if !storage_tries.contains_key(&hashed_address) {
                            ...
                            let storage_trie = if storage_root == mpt::EMPTY_ROOT || storage_root == B256::ZERO {
                                MptNode::default()
                            } else {
    ...
    fn calculate_state_root(
        ...
        let local_state = HashedPostState {
            ...
            storages: state.storages.into_iter()
                .map(|(k, v)| {
