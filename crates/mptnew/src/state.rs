...
pub struct EthereumState {
    ...
    pub address_keccak_cache: HashMap<Address, B256>,
    pub slot_keccak_cache: HashMap<U256, B256>,
...
impl EthereumState {
    pub fn new() -> Self {
        ...
        Self {
            ...
            address_keccak_cache: HashMap::with_capacity_and_hasher(
                1,
                DefaultHashBuilder::default(),
            ),
            slot_keccak_cache: HashMap::with_capacity_and_hasher(1, DefaultHashBuilder::default()),
            bump,
        }
    }
    ...
    pub fn update_from_bundle_state(&mut self, bundle_state: &BundleState) -> Result<(), Error> {
        for (address, account) in &bundle_state.state {
            let hashed_address = self.address_keccak_cache.get(address).unwrap();
            ...
            if let Some(info) = &account.info {
                ...
                for (slot, value) in &account.storage {
                    let hashed_slot = self.slot_keccak_cache.get(slot).unwrap();
